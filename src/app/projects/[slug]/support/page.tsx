...
export async function generateMetadata({ params }: Params) {
  ...
  return { title: `Support · ${slug} · Cybersystema`, robots: { index: true, follow: true } };
}

export default async function ProjectSupportPage({ params }: Params) {
  const { slug } = await params;
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-14">
      ...
      <header className="space-y-1">
        <p className="font-mono text-xs uppercase tracking-[0.45em] text-cyan-300/70">{"// support //"}</p>
        <h1 className="font-display text-4xl tracking-[0.14em] text-cyan-100">{project.name} — Support</h1>
      </header>
      <article className="rounded-xl border border-cyan-300/20 bg-slate-950/55 p-5">
        <Markdown source={project.support_md} />
      </article>
    </main>
  );
}
